...
struct BackendShow: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
...
struct BackendLocal: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
        commandName: "local",
...
struct BackendHosted: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
        commandName: "hosted",
    ...
    func run() async throws {
        do {
            ...
            FileHandle.standardError.write(Data("[backend]  checking endpoint...\n".utf8))
    ...
    private func checkEndpoint(baseURL: String, apiKey: String) async throws {
        ...
        guard let url = URL(string: "\(trimmed)/models") else {
            throw SiftError("invalid base URL")
        }
